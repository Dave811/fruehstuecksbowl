...
export default function LayersTab() {
  ...
  function startEdit(l: Layer) {
    setEditing(l)
    setForm({ name: l.name, sort_order: l.sort_order, selection_type: l.selection_type, quantity_options: l.quantity_options ?? '' })
  }
      ...
      <CardContent className="space-y-4">
        <form onSubmit={e => { e.preventDefault(); save(); }} className="space-y-4">
          ...
          <div className="flex gap-2">
            ...
            {editing && <Button type="button" variant="outline" onClick={() => { setEditing(null); setForm({ name: '', sort_order: 0, selection_type: 'single', quantity_options: '' }); }}>Abbrechen</Button>}
          </div>
        </form>
        <ul className="list-none p-0 m-0 border-t border-border pt-4 space-y-2">
          {layers.map(l => (
            <li key={l.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
              <span className="text-foreground"><strong>{l.name}</strong> (Reihe {l.sort_order}, {SELECTION_OPTIONS.find(o => o.value === l.selection_type)?.label ?? l.selection_type}{l.quantity_options ? `: ${l.quantity_options}` : ''})</span>
